...
class QuestionParser:
    ...
    def _load_keyword_patterns(self) -> dict[ProblemType, list[str]]:
        return {
            ...
            ProblemType.OPTIMIZATION: ["optimize", "maximize", "minimize", "best", "knapsack", "allocate", "anneal", "genetic", "hill climbing", "ant colony", "particle swarm", "swarm", "simulated annealing", "greedy"],
            ProblemType.ML: ["train", "model", "learn", "neural", "xgboost", "random_forest", "lightgbm", "gmm", "pca", "dimensionality", "reduce dimension", "t-sne", "tsne", "dimensionality reduction", "pca reduction", "fit", "predict"],
            ProblemType.NLP: ["text", "sentence", "semantic", "corpus", "language", "tokenize", "sentiment", "summariz", "enrich", "expand", "visualize word", "arithmetic", "rag", "retrieval", "retrieve", "analogy", "terminology", "vocabulary", "word2vec", "embedding", "bert", "transformer"],
            ProblemType.COMPUTER_VISION: ["detect", "segment", "recognize", "object", "satellite", "classify image", "cnn"],
            ProblemType.IMAGE_PROCESSING: ["blur", "filter", "denoise", "edge", "image", "satellite", "template", "pattern", "morpholog", "segment", "gaussian", "sharpen", "brighten", "contrast"],
            ProblemType.CLUSTERING: ["cluster", "group", "partition", "kmeans", "dbscan", "k-means", "agglomerative"],
            ProblemType.CLASSIFICATION: ["classify", "categorize", "label", "predict class", "logistic_regression", "knn", "svm", "naive_bayes", "decision_tree", "risk tier", "k-nearest"],
            ProblemType.REGRESSION: ["predict", "regress", "predict value", "forecast", "estimate", "linear_regression", "ridge", "lasso", "growth rate", "linear regression", "polynomial"],
            ProblemType.SEARCH: ["find", "search", "locate", "match", "retrieve", "lookup", "index"],
            ProblemType.GENERATION: ["generate", "create", "produce", "synthesize", "creative", "compose", "write"],
            ProblemType.SCHEDULING: ["schedule", "timetable", "assign", "allocate time", "deadline", "task order"],
        }
    ...
    def parse(self, question: str, data: Any = None) -> ProblemSpec:
        ...
        problem_type, confidence = self._detect_problem_type(question_lower)
    ...
    def _detect_problem_type(self, question: str) -> tuple[ProblemType, float]:
        ...
        if scores:
            ...
            max_score = scores[best_pt]
            sorted_scores = sorted(scores.values(), reverse=True)
            second_score = sorted_scores[1] if len(sorted_scores) > 1 else 0
            confidence = (max_score - second_score) / (max_score + 1e-6)
            if confidence >= self.CONFIDENCE_THRESHOLD:
                return best_pt, confidence
            ...
            if clf is not None:
                ...
                try:
                    ...
                    if result["scores"][0] > 0.5:
                        return ProblemType(result["labels"][0]), result["scores"][0]
                except Exception:
                    pass
