...
class AlgorithmEmbedder:
    ...
    def add_embedding(self, algo_name: str, embedding: torch.Tensor) -> None:
        self._cache[algo_name] = embedding

    def embed_algorithm(self, algorithm: Any) -> torch.Tensor:
    ...
    def _infer_problem_types(self, algorithm: Any) -> list[str]:
        name = algorithm.name.lower()
        tags = [t.lower() for t in getattr(algorithm, 'tags', [])]
        all_text = [name] + tags
        inferred = []

        def kw_in(keywords: list[str]) -> bool:
