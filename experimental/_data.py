...
def _normalize_numpy(data: Any, np: Any) -> Any:
    if data.ndim == 0:
        return data.item()
    return data.tolist()


def _normalize_pandas_df(data: Any) -> Any:
...
def _normalize_networkx_graph(data: Any, nx: Any) -> Any:
...
def _materialize_generator(gen: Any, cap: int = 10000) -> list:
    result = []
    try:
