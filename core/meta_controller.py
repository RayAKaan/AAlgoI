...
class UniversalMetaController:
    ...
    def _load_pretrained_model(self):
        ...
        from core.checkpoint_downloader import (
    ...
    def select(self, context: dict, candidates: list = None,
               problem_spec: ProblemSpec = None, query: str = ""):
        ...
        if self.kg_enabled and problem_spec:
            ...
            if len(semantic_candidates) < 3:
                self._cross_domain_pool = self.kg.find_cross_domain_candidates(
        ...
        if rl_ready:
            ...
            action_idx, log_prob, value = self.rl_agent.select_action(
                state,
            ...
            if candidate_mask is not None and raw_name not in pool_names:
                best_kg = self._select_best_algorithm(kg_pool, query, context)
                algo_name = best_kg if best_kg and best_kg in self.registry else raw_name
            else:
                algo_name = raw_name
            ...
            # If RL confidence is low and there's query context, override with scoring
    ...
    def _add_cross_domain_edges(self, kg):
        pattern_to_problems = {
