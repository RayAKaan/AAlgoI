...
class KNNAlgo(MLAlgorithm):
    ...
    def solve(self, data: dict) -> dict:
        ...
        X_test = np.array(data.get("X_test", data.get("X", [])))

        n_samples = len(X_train)
        n_neighbors = min(data.get("n_neighbors", 5), n_samples)
        ...
        predictions = model.predict(X_test_scaled).tolist() if len(X_test) > 0 else []

        return {
            "predictions": predictions,
            "accuracy": float(model.score(X_train_scaled, y_train)),
            "valid": True,
        }
